...
import { usePluginStoreApi } from '../../../stores/canvas/use-plugin-store';
...
export function usePluginHost(params: PluginHostParams) {
  ...
  const { requestEdit, requestGeneration, requestImageQuestion, requestVideoGeneration } = generationRuntime;
  const {
    effectiveConfig,
    isAiConfigReady,
    openConfigDialog,
  ...
  const pluginAi = useMemo<CanvasPluginAi>(() => {
    ...
    return {
      ...
      generateVideo: async (prompt, options) => {
        ...
        ensureReady(config);
        const file = await requestVideoGeneration(config, prompt, toReferences(options?.references), [], [], {
      ...
      generateText: async (prompt, options) => {
        ...
        const text = await requestImageQuestion(config, messages, (delta) => options?.onDelta?.(delta), {
          signal: options?.signal,
        });
        return { text };
      },
      ...
      listModels: (capability) =>
        selectableModelsByCapability(effectiveConfig, capability as ModelCapability | undefined).map((value) => ({
          value,
  ...
  }, [effectiveConfig, generationRuntime, isAiConfigReady, openConfigDialog, t]);
  ...
  const pluginHost = useMemo<CanvasPluginHost>(
    () => ({
      ...
      updateMetadata: (nodeId, patch) =>
        setNodes((prev) =>
          prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, ...patch } } : node)),
        ),
    ...
    [applyAgentOps, pluginAi, storageKeyPrefix],
  );
  ...
  useEffect(() => {
    void ensurePluginsLoaded(pluginStore);
  }, [pluginStore]);
