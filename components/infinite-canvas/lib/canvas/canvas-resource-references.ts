...
import { imageReferenceLabel } from '../image-reference-prompt';
import { getDataUrlByteSize, readImageMeta } from '../image-utils';
import { seedanceReferenceLabel } from '../seedance-video';
        ...
        const meta =
          metadata?.naturalWidth && metadata.naturalHeight
            ? {
                ...
                mimeType: metadata.mimeType || dataUrl.match(/^data:([^;]+)/)?.[1] || 'image/png',
              }
...
export function getMentionResourceNodes(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
  if (nodes.find((node) => node.id === nodeId)?.metadata?.musicResultSource || nodes.find((node) => node.id === nodeId)?.metadata?.musicSourceNodeId) return getContextResourceNodes(nodeId, nodes, connections);
...
function getContextResourceNodes(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[], visited = new Set<string>()): CanvasNodeData[] {
  if (visited.has(nodeId)) return [];
  const nextVisited = new Set(visited).add(nodeId);
  const target = nodes.find((node) => node.id === nodeId);
  return connections.filter((connection) => connection.toNodeId === nodeId).flatMap((connection) => {
    // A result inherits its generator's inputs, never the generator's selected output.
    if (target?.metadata?.musicSourceNodeId === connection.fromNodeId)
      return getContextResourceNodes(connection.fromNodeId, nodes, connections, nextVisited);
