...
import { Button, Image } from '../../runtime/ui/source-ui';
...
type CanvasConfigComposerProps = {
  value: string;
  musicConfig?: AiConfig;
  onMusicChange?: (values: CanvasMusicValues) => void;
...
export function CanvasConfigComposer({ value, inputs, mode, onChange, onClose, musicConfig, onMusicChange }: CanvasConfigComposerProps) {
  const config = useEffectiveConfig();
  const musicFields = mode === 'music' ? musicConfig?.modelAdapter?.music?.getComposerParameters?.(musicConfig, { audioCount: inputs.filter((input) => input.type === 'audio').length }) : undefined;
  ...
  const candidates = useMemo(() => {
    ...
    return inputs.filter((input) =>
      `${resourceLabel(input, inputs)} ${input.title} ${input.text || ''}`.toLowerCase().includes(query),
    );
  }, [inputs, mention]);
  ...
  }, [inputs, referenceById, theme, tokens, mode, musicFields?.length]);
  ...
  return (
    ...
    >
      ...
      <ConnectedInputList inputs={inputs} theme={theme} />
      {musicFields?.length && musicConfig && onMusicChange ? <CanvasMusicComposerFields config={musicConfig} fields={musicFields}
        prompt={value} connectedText={inputs.filter((input) => input.type === 'text').map((input) => input.text || '').join('\n\n')}
        onPromptChange={onChange} onMusicChange={onMusicChange} /> : <div className='relative rounded-xl'>
        {!value.trim() ? (
          <div
            className='pointer-events-none absolute left-3 top-2 text-sm leading-7'
            style={{ color: theme.node.placeholder }}
          >
            {mode === '3d' ? config.modelAdapter?.threeD?.label : t(`canvas.promptPanel.${mode}`)}
          </div>
        ) : null}
        <div
          ref={editorRef}
...
function ConnectedInputList({
  ...
  return (
    <div className='thin-scrollbar mb-2 flex max-w-full gap-1.5 overflow-x-auto pb-1' data-canvas-connected-inputs>
      {inputs.map((input) => (
        <div
          key={input.nodeId}
          className='flex min-w-28 max-w-44 shrink-0 items-center gap-1.5 rounded-lg border p-1.5 text-xs'
          style={chipStyle(theme)}
          title={input.text || input.title}
        >
          <ResourcePreview input={input} compact />
          <span className='min-w-0 flex-1'>
            <span className='block font-medium'>{resourceLabel(input, inputs)}</span>
...
function ResourcePreview({ input, compact = false }: { input: NodeGenerationInput; compact?: boolean }) {
  const sizeClassName = compact ? 'size-7' : 'size-9';
  if (input.type === 'image' && input.image)
    return <img src={input.image.dataUrl} alt='' className={`${sizeClassName} rounded-md object-cover`} />;
  if (input.type === 'video' && input.video)
    return (
      <video src={input.video.url} className={`${sizeClassName} rounded-md bg-black object-cover`} muted preload='metadata' />
    );
  const Icon =
    input.type === 'audio' ? Music2 : input.type === 'video' ? Video : input.type === 'image' ? ImageIcon : FileText;
  return (
    <span className={`grid ${sizeClassName} shrink-0 place-items-center rounded-md bg-black/10`}>
