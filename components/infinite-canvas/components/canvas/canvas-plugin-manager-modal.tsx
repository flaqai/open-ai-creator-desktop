...
import { App, Button, Input, Modal, Popconfirm, Switch, Tabs } from '../../runtime/ui/source-ui';
import { usePluginStore, usePluginStoreApi, type InstalledPlugin } from '../../stores/canvas/use-plugin-store';
...
export function CanvasPluginManagerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const pluginStore = usePluginStoreApi();
  ...
  const handleInstallUrl = async () => {
    ...
    setInstalling(true);
    try {
      const plugin = await installPluginFromUrl(pluginStore, target);
      message.success(t('canvas.plugins.installedPlugin', { name: plugin.name }));
      setUrl('');
    } catch (error) {
      message.error(
        t('canvas.plugins.installFailed', { error: error instanceof Error ? error.message : String(error) }),
      );
    } finally {
      setInstalling(false);
    }
  ...
  const handleInstallOfficial = async (entry: OfficialPluginEntry) => {
    setBusyId(entry.id);
    try {
      const plugin = await installPluginFromUrl(pluginStore, entry.url, { official: true });
    ...
    <>
      <Switch
        ...
        onChange={(checked) =>
          runOnPlugin(
            record,
            () => setPluginEnabled(pluginStore, record, checked),
      ...
      {!record.local && (
        <>
          <Button
            ...
            onClick={() =>
              runOnPlugin(record, async () => void (await updatePlugin(pluginStore, record)), t('canvas.plugins.updated'))
            }
          />
          <Popconfirm
            ...
            onConfirm={() => uninstallPlugin(pluginStore, record.id)}
          >
            <Button
              type='text'
              size='small'
  ...
  const withUpgradeDot = (icon: ReactNode) => (
    <span className='relative inline-flex'>
      ...
      <span
        ...
        style={{ background: 'var(--ui-green-color)', boxShadow: `0 0 0 2px ${theme.node.fill}` }}
    ...
    <div className='space-y-2'>
      <div className='flex items-center justify-between'>
        ...
        <Button
          type='text'
          size='small'
          icon={<RefreshCw className={`size-4 ${loadingOfficial ? 'animate-spin' : ''}`} />}
    ...
    <Modal title={t('canvas.plugins.title')} open={open} onCancel={onClose} footer={null} centered width={640}>
      <div className='space-y-3'>
        <div
          ...
          style={{
            borderColor: 'color-mix(in srgb, var(--ui-yellow-color) 34%, transparent)',
            background: 'color-mix(in srgb, var(--ui-yellow-color) 8%, transparent)',
