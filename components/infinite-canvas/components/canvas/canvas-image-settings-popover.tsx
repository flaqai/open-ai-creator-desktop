...
import { ImageSettingsPanel, imageQualityLabel, imageResolutionLabel, imageSizeLabel } from '../image-settings-panel';
...
export function CanvasImageSettingsPopover({
  ...
  const version = config.imageVersion || config.quality;
      ...
      <span ref={buttonRef} className='inline-flex min-w-0'>
        ...
        >
          <span className='truncate'>
            {imageResolutionLabel(config.imageResolution)} · {imageSizeLabel(activeSize)}
            {version ? ` · ${imageQualityLabel(version)}` : ''}
          </span>
        </Button>
      </span>
      {panel}
