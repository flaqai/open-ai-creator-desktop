...
export function useImageEditorViewport(image: ImageSize | null, open: boolean) {
  ...
  const panRef = useRef<{ pointerId: number; x: number; y: number; scrollLeft: number; scrollTop: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (document.activeElement instanceof HTMLElement && document.activeElement.matches("button,a,[role='button']"))
