...
type InfiniteCanvasProps = {
  ...
  onCanvasContextMenuRequest?: (clientX: number, clientY: number) => void;
...
export function InfiniteCanvas({
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const isRightButtonPan = event.button === 2 && tool === 'select';
    const shouldPan =
      event.button === 1 || isRightButtonPan || (event.button === 0 && activeTool === 'pan' && !isInteractiveTarget);

    if (shouldPan) {
      // Keep a stationary right click eligible for the existing context menu; movement suppresses it in capture.
      if (!isRightButtonPan) event.preventDefault();
      if (!isRightButtonPan) event.currentTarget.setPointerCapture(event.pointerId);
      panState.current = {
        isPanning: true,
  ...
  useEffect(() => {
    ...
    const handlePointerUp = () => {
      ...
      if (!panState.current.hasMoved && panState.current.startedOnBackground && panState.current.button !== 2) {
        onCanvasDeselect?.();
      }
      if (!panState.current.hasMoved && panState.current.startedOnBackground && panState.current.button === 2) {
        onCanvasContextMenuRequest?.(panState.current.startX, panState.current.startY);
      }
      panState.current.isPanning = false;
      setIsPanning(false);
      document.body.style.cursor = '';
  ...
  }, [onCanvasContextMenuRequest, onCanvasDeselect, onViewportChange]);
