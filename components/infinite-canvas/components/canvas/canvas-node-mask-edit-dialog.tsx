...
import { Button, Input, Modal, Slider, Tooltip } from '../../runtime/ui/source-ui';
...
const maskFillFallback = 'rgb(47 128 255)';
    ...
    >
      <div className='grid gap-5 lg:grid-cols-[minmax(360px,1fr)_320px]' data-canvas-no-zoom>
        ...
        {brushPreview
          ? createPortal(
              <div
                className={`pointer-events-none fixed z-[1100] rounded-full border-2 ${brushPreview.adjusting ? 'border-yellow-color bg-overlay-color/10' : 'border-on-media-color/90 bg-overlay-color/5'} shadow-[0_0_0_1px_rgba(0,0,0,.8)]`}
                style={{
                  left: brushPreview.x,
                  top: brushPreview.y,
        ...
        <div className='flex min-h-[360px] flex-col gap-5'>
          ...
          <div className='space-y-2'>
            ...
            />
            {error ? <div className='text-destructive-color text-xs font-medium'>{error}</div> : null}
          </div>
...
function configurePreviewStrokeContext(context: CanvasRenderingContext2D, stroke: MaskStroke) {
  ...
  const themeColor = rootStyle.getPropertyValue('--ui-canvas-accent').trim() || rootStyle.color;
  context.strokeStyle = themeColor || maskFillFallback;
  context.fillStyle = themeColor || maskFillFallback;
}
