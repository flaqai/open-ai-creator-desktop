...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(
  function CanvasResourceMentionTextarea(
    ...
    const mergedStyle = {
      ...
      ...(showOverlay
        ? { position: 'relative', zIndex: 1, background: 'transparent', backgroundColor: 'transparent' }
        : {}),
    } as CSSProperties;
    const menu =
    ...
    <>
      {value.split(pattern).map((part, index) =>
        labels.includes(part) ? (
          <span
            key={`${part}-${index}`}
            className='rounded-md bg-canvas-muted-accent/15 px-1 py-0.5 font-medium text-canvas-accent ring-1 ring-canvas-muted-accent/25'
          >
            {part}
          </span>
        ) : (
...
function MentionMenu({
  ...
  const boundary = textarea.closest('[data-canvas-overlay]')?.getBoundingClientRect() || {
    left: 8,
    top: 8,
  ...
  const showAbove =
    caretTop + lineHeight + gap + maxMenuHeight > boundary.bottom && caretTop - gap - maxMenuHeight >= boundary.top;
  const top = clamp(
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
  ...
  if (reference.kind === 'video' && reference.previewUrl)
    return (
      <video src={reference.previewUrl} className='size-9 rounded-md bg-black object-cover' muted preload='metadata' />
    );
  const Icon =
