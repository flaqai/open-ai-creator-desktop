...
import { Button, InputNumber, Modal, Tooltip } from '../../runtime/ui/source-ui';
...
export function CanvasNodeSplitDialog({
  ...
  const pieceSize = image
    ? { width: Math.max(1, Math.floor(image.width / columns)), height: Math.max(1, Math.floor(image.height / rows)) }
    : null;
  ...
  const update = (key: 'rows' | 'columns', value: string | number | null) => {
    ...
    setParams((current) => ({
      ...current,
      [key]: count,
      [key === 'rows' ? 'horizontalLines' : 'verticalLines']: buildGridLines(count),
    }));
  };
  ...
  const deleteLine = () => {
    ...
    setParams((current) => {
      ...
      return {
        ...
        columns: active.axis === 'vertical' ? lines.length + 1 : current.columns,
      };
    });
    setActive(null);
  };
  const startDrag = (axis: 'horizontal' | 'vertical', index: number, event: ReactPointerEvent) => {
    ...
    const move = (moveEvent: PointerEvent) =>
      setLine(
        axis,
        index,
        axis === 'horizontal' ? (moveEvent.clientY - box.top) / box.height : (moveEvent.clientX - box.left) / box.width,
      );
          ...
          <div className='space-y-5 py-2'>
            ...
            <div className='rounded-xl border px-4 py-3 text-sm'>
              ...
              <div className='mt-2 flex items-center justify-between'>
                ...
                <span className='font-semibold'>
                  {pieceSize ? `${pieceSize.width} x ${pieceSize.height}` : t('canvas.editors.unknown')}
                </span>
              </div>
            </div>
            <Button
  ...
  return (
    <div className='pointer-events-none absolute inset-0'>
      {verticalLines.map((line, index) => (
        ...
        >
          <div
            className={`absolute left-1/2 top-0 h-full border-l shadow-[0_0_0_1px_rgba(0,0,0,.35)] ${active?.axis === 'vertical' && active.index === index ? 'border-yellow-color' : 'border-on-media-color/90'}`}
          />
        </div>
      ))}
      {horizontalLines.map((line, index) => (
        ...
        >
          <div
            className={`absolute left-0 top-1/2 w-full border-t shadow-[0_0_0_1px_rgba(0,0,0,.35)] ${active?.axis === 'horizontal' && active.index === index ? 'border-yellow-color' : 'border-on-media-color/90'}`}
          />
        </div>
      ))}
    </div>
