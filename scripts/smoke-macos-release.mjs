...
export async function smokeMacRelease({ dmg, version, arch }) {
  ...
  try {
    ...
    const architectures = execFileSync('/usr/bin/lipo', ['-archs', executable], { encoding: 'utf8' })
      .trim()
      .split(/\s+/);
    ...
    console.log(`PASS ${path.basename(dmg)}: ${version}, ${plist.CFBundleIdentifier}, ${arch}, startup stable.`);
  } finally {
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const [dmg, version, arch] = process.argv.slice(2);
  if (!dmg || !version || !arch) {
    throw new Error('Usage: node scripts/smoke-macos-release.mjs <dmg> <version> <arm64|x86_64>');
  }
  await smokeMacRelease({ dmg, version, arch });
}
